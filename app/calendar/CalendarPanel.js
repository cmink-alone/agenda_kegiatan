...
Ext.define('Ext.calendar.CalendarPanel', {
    ...
    initComponent: function() {
        ...
        if(this.showTitleBar) {
            var month_menu = Ext.create('MP.view.MonthPicker', {
                calpanel: this
            });
            var button = {
                id: this.id + '-tb-title',
                //text: '<B>' + Ext.Date.format(new Date(), 'F Y') + '</B>',
                menu: month_menu
            }

            this.tbar.items.push(button);
        }
    ...
    fireViewChange: function() {
        if (this.layout && this.layout.getActiveItem) {
            ...
            this.down('[id=' + this.id + '-tb-title]').setText('<B>' + Ext.Date.format( this.startDate, 'F Y') + '</B>');
